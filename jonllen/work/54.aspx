...
<head>
<title>PowerDesigner使用(修改版)—时代财富技术部培训资料 - 工作 - 金龙博客</title>
<meta name="description" content="PowerDesigner使用
    ...
    <div class="articlecont">
        <div class="position">
		    ...
		    <a href="http://www.jonllen.com">首页</a>
		     ...
		     > <a href="/jonllen/work/54.aspx">PowerDesigner使用(修改版)—时代财富技术部培训资料</a>
		</div>
        <div class="article">
			<div class="title"><h2>PowerDesigner使用(修改版)—时代财富技术部培训资料</h2></div>
...
<h1>PowerDesigner使用</h1>
<p style="text-indent: 2em;">PowerDesigner是一种著名的CASE建摸工具，最开始为数据库建模设计，即物理模型(Physical Data Model)用于生成数据库表结构，还有面向对象模型(Object Oriented Model)，用于建立UML模型的结构，可以直接生成CS代码，还有其他的模型等等，不同的模型之间可以相互的转化。我觉得它最实用方便的功能是使用物理模型直接设计生成数据库，给我们对数据库的生成升级维护带来极大的方便，下面主要来讲一下它的物理模型设计。</p>
<p style="text-indent: 2em;"><strong>1.使用PowerDesigner物理模型生成数据库</strong></p>
<p style="text-indent: 2em;">PowerDesigner工程是采用Workspace工作空间，一个工作空间可包含多个模型项目，每个模型项目对应一个数据库结构。打开PowerDesigner在File里New选择Physical Data Model(物理模型)，可在DBMS选项里面选择数据库的类型，新建立模型后PowerDesigner自动创建一个工作空间(Workspace)放在里面，然后模型下面默认会创建一个名PhysicalDiagram_1的物理图表区，这个其实就是用来区分数据库内某一模块的单元区域，我一般是把数据库内有关系的表放在同一块里面，如会员模块就叫MemberDiagrm里面放会员及其相关的表，其他模块就另外新建一个另外放在一起。浮在IDE右上方的Palette就是PowerDesigner里一些常见操作的工具面板，点击选择<img title="table" src="/upload/jonllen/article/a_pd_table.gif" width="16" height="15" />在图表区新建一个FA_Member会员表，双击表在General里面修改表名描述，这里的Name是显示名最好用中文，然后Code就是数据库的表名一般用英文，默认是Name to Code mirroring名字会连动的，如果觉得这个功能有点讨厌可在Tools工具栏General Options里Dialog的Name to Code mirroring不选中即可。表的列是在Columns选项卡里面添加的，同样Name最好用中文描述而Code即是表的列名了。 后面的P、F、M选项分别代表主键(Primary)、外键(Foreign Key)、强制不能为空(Mandatory)，选中主键Mandatory也会自动选中，外键不可选当字段有外键引用时自动选中。双击字段列可修改字段的一些属性，有描述、字段类型等，字段默认值在Standard Checks选项卡的Values里Default处设置。再新建一个会员类型表FA_MemberType，此时会员表里面的会员类型字段应该是引用该会员类型表里面的会员类型编号，在PowerDesigner里表的主外关系建立的方法是：在Palette工具面板里面选择<img title="reference" src="/upload/jonllen/article/a_pd_reference.gif" alt="reference" width="16" height="15" />Reference后先点击从表然后再拖向主表，从表一条带箭头实线指向主表以表示主外键引用关系，双击关系名以编辑引用关系属性，建议主外键关系取名为：FK_从表_Reference_主表，但注意这里的Code不是数据库内生成的主外键关系名，而是在Integrity(完整性)选项卡里Constraint name设置，另外对Insert和Update强制关系级联(Cascade)更新、删除操作(sql2005叫叠层并可以设置为空或默认值)也在此选项卡内设置。Join选项卡里是设置两个主外键表的引用列，PowerDesigner会自动选择主表主键为默认主外键列，如果两个表的列名不一样则需要修改，无论是修改主外键还是表都能在Preview选项卡里面即时预览看到生成的SQL语句。生成整个数据库的SQL语句是在Database工具栏的Generate Database里，建议新建一个sql目录专门放生成的sql语句文件，第一次创建数据库所有表建议取名叫create1.0.sql(因为后面全部是update)，Generation type建议选择Script generation生成sql脚本文件，而不要选择下面的Direct generation直接连接数据库执行，因为有时候生成的SQL语句里面会有Warn警告需要稍做修改不然会报错的(如修改表字段类型时候将以前表内数据导入时会有Warn)。Automatic archive自动备份也需要选中，该选项使生成sql语句后将弹出保存当前工程模型的对话框，后缀名为apm(其实内容和工程文件格式pdm是一样的，同样都可以打开)，建议新建一个目录叫log里面存所有apm格式的归档记录文件，由于是创建数据库文件所以建议取名叫create1.0.apm，这样能与创建数据库的sql文件一一对应。要创建数据库，找到sql目录里生成的create1.0.sql文件执行即可。</p>
<p style="text-indent: 2em;"><strong>2.使用PowerDesigner升级维护数据库</strong></p>
<p style="text-indent: 2em;">对于以前老系统我们有时需要对数据库进行升级维护，比如在原来基础上加上一些表或修改字段等。在PowerDesigner里的操作稍微比创建时麻烦点，还是继续以上面会员为例子，假设我们的系统数据库原来是用PowerDesigner设计出来的然后有以上模型结构，现在要在会员表上面增加一个区域编号，并引用新建的区域表做外键，那用PowerDesigner模型该如何操作呢？首先开打pdm工程文件，在以前模型基础上直接进行修改，这里的操作就是新建一个区域表在再会员表添加一个区域字段引用它的外键，所有的修改完成后在Database工具栏Modify Database里面进行生成修改后的SQL语句，仍然建议设置生成sql语句的目录放在之前建立的sql目录下面，但是现在的文件名应该叫update2.0.sql，或者加在后面加上括号简要描述，如update2.0(add area).sql。注意记得指定Backup tables和Automatic archive选项，选中Backup tables将在修改表结构之前把原表改名加个tmp_前缀，创建好新的表结构后再insert into from select tmp_表插入以前表内的数据，起到备份以前表内数据功能，这就是为什么有时候用PowerDesigner升级数据库后会多一些tmp开头的临时表了，Automatic archive是在生成升级sql语句后提示保存当前记录的选项。由于这次修改是针对上创建数据库时候的修改，注意指定Get database schema选项里的Using a archive model文件，这里这个文件的路径应该是log目录下面的create1.0.apm文件，因为这次升级是针对上次创建数据库时的一次升级，PowerDesigner会自动拿当前模型和create1.0.apm之前保存的历史存档模型比较生成升级的sql语句，并显示确切的修改对比变化，很清晰一目了然，如下图：</p>
<div style="text-indent: 2em;"><a href="/upload/jonllen/article/a_pd_different.gif" target="_blank"><img title="Modify Different" src="/upload/jonllen/article/a_pd_different.gif" width="500" height="406" /></a></div>
<p style="text-indent: 2em;">最后提示保存Archive Model归档记录模型，这里应该叫update2.0.amp(或加上括号简要描述)并保存到log目录下面，同样与update2.0对照，这样命名条理清晰可读性强。以后的升级依此类推，都是相对之前的归档记录进行升级，升级后记得保存当前记录，如果变化不大可以叫update2.1,update2.2依次递增，如果修改较大可以升级一个版本序号叫update3.0，或者后面加上括号和简要的描述等。但是有时候我们修改的地方很少产生的update修改语句可能不会马上去执行，而是要等到下一次有多个修改的地方合起来一起去更新，这样我们的话我们就有可能有多个update语句文件在服务器数据库上还没有执行，但是和以前的update语句却区分不开来到底有没有升级过的，虽然说PowerDesigner产生的升级语句是可以多次执行的，但不建议这么多，因为这样有可能会出错，有个办法就是在每个已经升级过的sql文件名后加个后缀_ed表示已经升级过的，这样我下次升级后就只执行那些没有_ed还没有升级过的sql文件。</p>
...
<p style="text-indent: 2em;">这样，PowerDesigner对数据库管理升级操作的的介绍就到此为止，以后发现新的功能再补上。相比Rose我觉得PowerDesigner它的功能更方便实用，而Rose可谓身出名门D调华丽，为Java量身打造很多功能，现在归属于IBM。PowerDesigner也历经风波最终被美国的Sybase所收购，这里值得一提的是PowerDesigner最初是由一个中国人开发创建，中国软件加油!!!</p>
<p style="text-indent: 2em;">以上PowerDesigner事例下载：<a href="/upload/jonllen/case/PowerDesign使用(修改).rar" style="color: #ff6600;">Case下载</a> 事例版本：PowerDesigner 12.5</p>
</div>
			</div>
			...
			<div class="tags">标签：<a href="/jonllen/work/">工作</a> PowerDesigner,时代财富
			...
			<div class="desc">
			  <ul>
				<li>posted@ 2009-08-10 10:21</li>
				<li>update@ 2010-02-11 15:42:26</li>
				<li>阅读(<span id="articleClick">15182</span>)</li>
				<li>评论(0)</li>
...
<div class="context">
			<ul>
				<li>上一篇：<a href="/jonllen/work/35.aspx">我的工作记录</a></li>
<li>下一篇：<a href="/jonllen/work/56.aspx">软件开发规范—模块开发卷宗(GB8567——88)</a></li></ul>
		</div>
...
<div class="correlative">
	...
	暂无任何评论。
        ...
        <script type="text/javascript">
		    ...
		    {
		        var url = "/plugin/web/doSaveComment.do?sourcetype=1&sourceid=54&siteId=1";
		        var data = "username="+encodeURIComponent(document.getElementById("txtUserName").value);
		        data += '&sourceurl=/jonllen/work/54.aspx';
...
document.getElementById('commentContent').appendChild(itemComment);
            ...
            function addClick()
		    {
		        var url = "/plugin/web/doAddClick.do?columnType=1&documentId=54";
		        Ajax.send({
		            type : "GET",
		            url : url,
		            fn : function (data){
