...
<head>
<title>Apache2.2+PHP5.2.13配置Moodle1.9.8+SQL2005 - 工作 - 金龙博客</title>
<meta name="description" content="由于最近工作的关系，需要研究Moodle系统，它是一个线上学习的教学平台，是一个PHP的免费开源项目，目前Windows下的最新版是Moodle 1.9.8，推荐使用MySql数据库，毕竟PHP+MySQL组合一起会更佳，但由于我目前还在MS的.Net阵营，所以期待使用熟悉的MSSQL数据库，这样也能跟现有的.Net系统进行很好的整合，并有利于以后的维护。配置好PHP运行环境..." />
		        ...
		        Ajax.send({
		            ...
		            fn : function (data){
	                    ...
	                    if (rs.success)
	                    {
	                        ...
	                        WdatePicker( { eCont :'calendar', firstDayOfWeek :1, specialDates :archive.length > 0 ? archive : null, onpicked :ePicked, startDate:'2010-04-13' } );
 	                    }else
	                    {
	                        alert(rs.error);
	                    }
    ...
    <div class="articlecont">
        <div class="position">
		    ...
		    <a href="http://www.jonllen.com">首页</a>
		     ...
		     > <a href="/jonllen/work/115.aspx">Apache2.2+PHP5.2.13配置Moodle1.9.8+SQL2005</a>
		</div>
        <div class="article">
			<div class="title"><h2>Apache2.2+PHP5.2.13配置Moodle1.9.8+SQL2005</h2></div>
			...
			<div class="cont" id="content">
				<p>由于最近工作的关系，需要研究Moodle系统，它是一个线上学习的教学平台，是一个PHP的免费开源项目，目前Windows下的最新版是<a href="http://download.moodle.org/windows/" target="_blank" title="Moodle下载">Moodle 1.9.8</a>，推荐使用MySql数据库，毕竟PHP+MySQL组合一起会更佳，但由于我目前还在MS的.Net阵营，所以期待使用熟悉的MSSQL数据库，这样也能跟现有的.Net系统进行很好的整合，并有利于以后的维护。</p>
<p><strong>配置好PHP运行环境</strong></p>
<p>从<a href="http://moodle.org/" target="_blank">Moodle</a>官网下载最新的安装包，解压后里就包含了Apache和PHP的运行文件，而且都是不需要重新安装的绿色版，如果想自动运行则可手动添加进Windows后台服务，进入Apache主目录的bin目录下，使用cmd命令安装Apache后台服务：httpd.exe -k install。</p>
<p><strong>PHP连MSSQL数据库</strong></p>
<p>确保Apache运行PHP没问题后，再需要修改PHP主目录下的php.ini配置文件，以支持MSSQL数据库的连接。为了更好的进行调试显示错误，先分别修改php.ini里的display_errors = On和log_errors = On显示错误信息，配置过程主要参考官网说明文档：<a href="http://docs.moodle.org/en/Installing_MSSQL_for_PHP" target="_blank" title="Installing_MSSQL_for_PHP">http://docs.moodle.org/en/Installing_MSSQL_for_PHP</a>，直到最后运行test.php提示连接数据库成功。</p>
<p><strong>修改Moodle的config.php</strong></p>
<p>先copy一份moodle安装目录下的config-dist.php文件，并另存为config.php。按照官网说明将$CFG-&gt;的dbtype、dbhost、dbname分别配置好，如果mssql为sql2005数据库，则dbhost应该包含实例名如：127.0.0.1\SQL2005。dbname可为空，将生成表结构到系统默认的master数据库内，建议先创建一个空数据库后再指定dbname。修改$CFG-&gt;的wwwroot、dirroot、dataroot配置，根据你本机的Apache配置而进行修改正确。</p>
<p><strong>提示连接数据库失败Error: Database connection failed.</strong></p>
<div style="text-align: center; color: #990000; border: solid 1px black;">
...
<p>这是我今天遇到的最多的错误信息，主要还是连接SQLSERVER数据库的问题，总结问题原因主要为如下几点：</p>
<ol>
<li>确保已下载了<a href="http://download.moodle.org/dblib/php52/DBLIB_TS.zip" target="_blank">php_dblib.dll</a>文件放在PHP主目录的ext目录下，并修改了php.ini增加extension=php_dblib.dll行，启动PHP时候如果提示Unable to load dynamic library './php_dblib.dll' - 找不到指定的模块，则修改php.ini的extension_dir为正确的绝对路径。</li>
<li>确保C:\freetds.conf文件已正确的创建。</li>
<li>检查config.php内的$CFG-&gt;dbtype、dbhost、dbname数据库连接信息已经正确的配置好。</li>
<li>检查SQL Server服务端配置，如登陆帐号权限、是否允许TCP/IP连接、是否开启了SQLBrowser(SQL Server Browser)服务。</li>
</ol>
<p><strong>Warning: mssql_connect() [function.mssql-connect]: Unable to connect to server</strong></p>
<p>这个警告很奇怪，是我在配置Moodle用户使用外部数据库时，登陆出现的错误信息。当我使用外部的MSSQL2005数据库表来检验用户名和密码时，明明设置的外部数据库表里有这条记录，却老是提示<strong>登录无效，请重试</strong>，运行test.php也提示Successful connection能成功连接到数据库，后面将外部数据库的登陆用户名设置为<strong>sa</strong>，问题解决。</p>
			</div>
			...
			<div class="desc">
			  <ul>
				<li>posted@ 2010-04-13 22:39</li>
				<li>update@ 2010-04-15 16:36:12</li>
				<li>阅读(<span id="articleClick">9964</span>)</li>
				<li>评论(0)</li>
...
<div class="context">
			<ul>
				<li>上一篇：<a href="/jonllen/work/111.aspx">天河部落网站升级改版工作</a></li>
<li>下一篇：<a href="/jonllen/work/116.aspx">Moodle系统整合方案</a></li></ul>
		</div>
...
<div class="correlative">
			...
			<ul>
                    	...
                    	<li><a href='/jonllen/work/116.aspx' title='' target="_blank">Moodle系统整合方案</a></li>
	...
	暂无任何评论。
        ...
        <script type="text/javascript">
		    ...
		    {
		        var url = "/plugin/web/doSaveComment.do?sourcetype=1&sourceid=115&siteId=1";
		        var data = "username="+encodeURIComponent(document.getElementById("txtUserName").value);
		        data += '&sourceurl=/jonllen/work/115.aspx';
...
document.getElementById('commentContent').appendChild(itemComment);
            ...
            function addClick()
		    {
		        var url = "/plugin/web/doAddClick.do?columnType=1&documentId=115";
		        Ajax.send({
		            type : "GET",
		            url : url,
		            fn : function (data){
