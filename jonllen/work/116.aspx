...
<head>
<title>Moodle系统整合方案 - 工作 - 金龙博客</title>
		        ...
		        Ajax.send({
		            ...
		            fn : function (data){
	                    ...
	                    if (rs.success)
	                    {
	                        ...
	                        WdatePicker( { eCont :'calendar', firstDayOfWeek :1, specialDates :archive.length > 0 ? archive : null, onpicked :ePicked, startDate:'2010-04-24' } );
 	                    }else
	                    {
	                        alert(rs.error);
	                    }
    ...
    <div class="articlecont">
        <div class="position">
		    ...
		    <a href="http://www.jonllen.com">首页</a>
		     ...
		     > <a href="/jonllen/work/116.aspx">Moodle系统整合方案</a>
		</div>
        <div class="article">
			<div class="title"><h2>Moodle系统整合方案</h2></div>
			...
			<div class="cont" id="content">
				<h4>一、项目背景</h4>
<p>原有Moodle系统环境：Linux debian + Apache/2.2.9 + PHP Version 5.2.6 + mysql 5.0</p>
<p>新天河部落系统环境 ：Win2003 + IIS6.0(.NET v2.0) + SqlServer2005</p>
<h4>二、项目需求</h4>
<p>根据天河部落优化合同(1209) (附件3)要求，对天河部落和天河Moodle进行整合，实现统一登录通行证功能，内容如下：</p>
<p>为达到一次注册多站访问的效果，拟通过统一登录通行证的方法实现，即天河Moodle使用天河部落数据库中的用户表。</p>
<p>根据Moodle所提供的身份验证方法，有效的结合天河部落现有用户数据，建议选用天河Moodle直接使用天河部落的数据库。该方法使用一个外部数据库来检验用户名和密码是否有效。如果是一个新帐号，该帐号其它字段的信息将一起复制到本系统中。</p>
<p>用户在天河Moodle注册，直接转跳到天河部落注册。</p>
<p>用户在天河Moodle登录，系统会在Moodle的用户数据表中查找是否存在该授权用户；若无，则链接到天河部落用户数据表查找；在天河部落用户数据表检查到该用户，则将该用户信息导入到Moodle的用户数据表；在天河部落用户数据表也没有该用户数据，则不允许登录。</p>
<h4>三、可选方案分析</h4>
<h6>1.在客户端发送HTTP请求自动登陆Moodel系统(目前已实施)</h6>
<p><strong>原理</strong></p>
<p>用户在登陆和注销Moodle系统的时，跳转到天河部落的登陆和注销页面，如果登陆成功则在客户端发送POST请求再模拟登陆Moodle系统，Moodle系统响应后跳转，完成整个登陆过程，如图1-1所示。</p>
<div><a href="/upload/jonllen/article/moodle_login.gif" target="_blank" title="点击查看大图"><img src="/upload/jonllen/article/moodle_login.gif" alt="图1-1(天河部落Moodle登陆流程)" width="500" /></a></div>
<p style="text-align: center;">图1-1(天河部落Moodle登陆流程)</p>
<p>Moodle系统的用户身份验证必须配置为使用外部数据库，如图1-2所示，这样才能保证登陆Moodle系统时天河部落已存在的用户记录会复制到本系统中，Moodle系统登陆页面已能实现上面的功能，先判断用户记录在本系统数据库中是否存在，如不存在再判断是否在外部数据库中存在，如存在则新插入一条记录到本系统，如都不存在则登陆失败。</p>
<div><a href="/upload/jonllen/article/moodle_outdatabase.JPG" target="_blank" title="点击查看大图"><img src="/upload/jonllen/article/moodle_outdatabase.JPG" alt="图1-2(天河部落Moodle使用外部数据库登陆) " width="500" /></a></div>
<p style="text-align: center;">图1-2(天河部落Moodle使用外部数据库登陆)</p>
<p><strong>优点</strong></p>
<ol>
<li>充分利用了Moodle的使用外部数据库功能；</li>
<li>对Moodle系统的改动较小，可保留原Moodle系统不必升级，只需要修改原系统的/login/index.php(登陆)和/login/logout.php(注销)页面；</li>
</ol>
<p><strong>缺点</strong></p>
<ol>
<li>目前只整合了用户的登陆和注销，还需要增加对修改密码等操作的用户同步功能；</li>
<li>扩展性较差，完全依赖于现有Moodle系统的本身功能，新增一个功能需要重新开发；</li>
<li>在IE里客户端发送POST请求再自动登陆Moodle系统如不是同一个域第一次会出现提示，如果用户选择&ldquo;否&rdquo;则登陆不了Moodle系统（给Moodle系统分配一个moodle.thjy.org好象不会出现该问题）；</li>
</ol>
<h6>2.跳转至Moodel系统登陆接口页面</h6>
<p><strong>原理</strong></p>
<p>在原Moodle系统新增一个interface.php页面，该页面根据传递的不同参数分别进行Moodle系统的登陆、注销和修改密码等操作，并对URL地址参数进行加密和验证，仍然需要修改原Moodle系统的登陆和注销页面跳转到天河部落的登陆和注销页面，但天河部落登陆成功后不再需要使用客户端发送POST请求模拟登陆Moodle系统，而直接在服务器端重定向到Moodle系统的interface.php页面，验证后再跳转回来，完成整个登陆过程。</p>
<p><strong>优点</strong></p>
<ol>
<li>较为安全和稳定，不受客户端脚本等安全级别设置的影响；</li>
</ol>
<p><strong>缺点</strong></p>
<ol>
<li>完成interface.php需深入了解Moodle系统，并要掌握一定的PHP知识；</li>
</ol>
<h6>三、其他方案...</h6>
			</div>
			...
			<div class="tags">标签：<a href="/jonllen/work/">工作</a> moodle
			...
			<div class="desc">
			  <ul>
				<li>posted@ 2010-04-24 11:38</li>
				<li>update@ </li>
				<li>阅读(<span id="articleClick">10718</span>)</li>
				<li>评论(0)</li>
...
<div class="context">
			<ul>
				<li>上一篇：<a href="/jonllen/work/115.aspx">Apache2.2+PHP5.2.13配置Moodle1.9.8+SQL2005</a></li>
<li>下一篇：<a href="/jonllen/work/119.aspx">2010上半年系统集成项目管理工程师软考试题分析</a></li></ul>
		</div>
...
<div class="correlative">
			...
			<ul>
                    	...
                    	<li><a href='/jonllen/work/115.aspx' title='' target="_blank">Apache2.2+PHP5.2.13配置Moodle1.9.8+SQL2005</a></li>
	...
	暂无任何评论。
        ...
        <script type="text/javascript">
		    ...
		    {
		        var url = "/plugin/web/doSaveComment.do?sourcetype=1&sourceid=116&siteId=1";
		        var data = "username="+encodeURIComponent(document.getElementById("txtUserName").value);
		        data += '&sourceurl=/jonllen/work/116.aspx';
...
document.getElementById('commentContent').appendChild(itemComment);
            ...
            function addClick()
		    {
		        var url = "/plugin/web/doAddClick.do?columnType=1&documentId=116";
		        Ajax.send({
		            type : "GET",
		            url : url,
		            fn : function (data){
