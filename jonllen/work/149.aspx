...
<head>
<title>基于UKey数字证书实现身份认证 - 工作 - 金龙博客</title>
<meta name="description" content="随着电子商务在线交易的流行，一些网上银行也推出了系列措施保证在线交易的安全，有使用软证书的，也有使用UKey硬证书的，这都是数字证书的身份认证的应用。下面我就说下从数字证书生成到身份认证实现的过程。" />
		        ...
		        Ajax.send({
		            ...
		            fn : function (data){
	                    ...
	                    if (rs.success)
	                    {
	                        ...
	                        WdatePicker( { eCont :'calendar', firstDayOfWeek :1, specialDates :archive.length > 0 ? archive : null, onpicked :ePicked, startDate:'2012-11-26' } );
 	                    }else
	                    {
	                        alert(rs.error);
	                    }
    ...
    <div class="articlecont">
        <div class="position">
		    ...
		    <a href="http://www.jonllen.com">首页</a>
		     ...
		     > <a href="/jonllen/work/149.aspx">基于UKey数字证书实现身份认证</a>
		</div>
        <div class="article">
			<div class="title"><h2>基于UKey数字证书实现身份认证</h2></div>
			...
			<div class="cont" id="content">
				<p>随着电子商务在线交易的流行，一些网上银行也推出了系列措施保证在线交易的安全，有使用软证书的，也有使用UKey硬证书的，这都是数字证书的身份认证的应用。下面我就说下从数字证书生成到身份认证实现的过程。</p>
<p><strong>一、PKI搭建</strong></p>
<p>一套完整的PKI（Public Key Infrastructure公钥基础设施）系统包括了KMC（密钥管理中心）、CA（Certificate Authority）、RA（Register Author注册审批机构），KMC负责密钥管理，CA是核心进行制作证书，RA系统提供证书业务申请审核。数字证书有RSA、ECC等算法的，按证书类型分个人证书、设备证书和机构证书，按用途有加密证书和签名证书。完善的CA系统一般会提供证书申请制作、撤消、冻结、解冻、续费、更新业务功能，还有相应的OCSP（在线证书状态协议）、CRL（证书撤消列表）查询服务等。Windows本身有可以集成CA，可在IIS内提供证书申请制作服务，这样搭建的CA适合较小范围数字证书应用的使用。PKI系统包括如下组成部分。</p>
<div><a href="/upload/jonllen/article/WGPKI.jpg" target="_blank" title="PKI系统"><img title="PKI系统" src="/upload/jonllen/article/WGPKI.jpg" alt="PKI系统" /></a></div>
<p><strong>二、证书激活</strong></p>
<p>向CA发送证书制作申请后，证书制作完成后怎么交付给客户使用呢？有一种方式是软证书，返回pfx标准格式的数字证书，导入到客户系统以供需要时选择使用。软证书是以文件形式保存的，并且可以标记允许再次导出，硬证书则是以UKey移动设备为载体，保存私钥和数字证书。</p>
<p>证书激活是指证书从CA制作出来到交付给客户以供使用的过程，软证书使用比较简单，一般CA在制作出证书会返回pfx标准格式的证书，只需要安装到系统。UKey硬证书则需要将私钥和数字证书导入到设备中，由于涉及到私钥安全保护，UKey写入证书私钥时会使用加解密，UKey证书激活过程一般包括设备连接、验证PIN码、初始化导出公钥、写入数据，整个流程如下图所示。</p>
<div><a href="/upload/jonllen/article/WGCertActive.jpg" target="_blank" title="UKey证书激活流程图"><img title="UKey证书激活流程图" src="/upload/jonllen/article/WGCertActive.jpg" alt="UKey证书激活流程图" /></a></div>
<p><strong>三、身份认证</strong></p>
<p>数字证书写入到UKey中怎么用？一般UKey设备厂商会提供API，另外还需要有相应的驱动程序，可以让系统识别到UKey移动设备，提供相应的加解密签名接口，如读取UKey序列号、读取加密签名证书、签名等，禁止直接读取私钥，并有相应安全策略保护。</p>
<p>通过UKey签名可以实现身份认证，因为UKey中包含的私钥就只有指定身份才能持有，拿使用UKey数字签名登录后台为例，首先插入UKey设备，然后签名随机生成的验证码，得到签名数据，再输入用户名一起提交到后台，后台先做验证码正确性检查，然后再通过用户名查询定位数据库中的用户签名证书记录，使用用户的签名证书对用户传过来的签名数据进行验签，如果验证通过则证明是对应的用户，从而实现身份认证过程。这种方式比传统的口令认证更安全，甚至可以不需要用户名就可以登录验证，用户名是可以保存在数字证书信息中的，并且证书的序列号在同个CA中也是唯一的。身份认证实现的基本流程如下。</p>
<div><a href="/upload/jonllen/article/WGAuthentication.jpg" target="_blank" title="UKey身份认证流程"><img src="/upload/jonllen/article/WGAuthentication.jpg" alt="UKey身份认证流程" /></a></div>
<p>UKey设备身份认证为作为可随身携带的智能密码钥匙，比使用软证书更安全快捷。UKey设备只是存储数字证书的一种介质，私钥在设备里是牢牢保护的，用户只需要保护好设备的使用安全，就可以达到更安全的目的。</p>
			</div>
			...
			<div class="desc">
			  <ul>
				<li>posted@ 2012-11-26 13:18</li>
				<li>update@ 2012-12-01 11:36:55</li>
				<li>阅读(<span id="articleClick">8236</span>)</li>
				<li>评论(0)</li>
...
<div class="context">
			<ul>
				<li>上一篇：<a href="/jonllen/work/148.aspx">论构建信息系统安全策略</a></li>
<li>下一篇：<a href="/jonllen/work/151.aspx">持续集成工具TeamCity配置使用</a></li></ul>
		</div>
...
<div class="correlative">
			...
			<ul>
                    	...
                    	<li><a href='/jonllen/work/148.aspx' title='' target="_blank">论构建信息系统安全策略</a></li>
	...
	暂无任何评论。
        ...
        <script type="text/javascript">
		    ...
		    {
		        var url = "/plugin/web/doSaveComment.do?sourcetype=1&sourceid=149&siteId=1";
		        var data = "username="+encodeURIComponent(document.getElementById("txtUserName").value);
		        data += '&sourceurl=/jonllen/work/149.aspx';
...
document.getElementById('commentContent').appendChild(itemComment);
            ...
            function addClick()
		    {
		        var url = "/plugin/web/doAddClick.do?columnType=1&documentId=149";
		        Ajax.send({
		            type : "GET",
		            url : url,
		            fn : function (data){
